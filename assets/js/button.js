class Button {
    ...
    deleteElement() {
        ...
        requestDelete.ajaxDelete(this.url, (response) => { //Je lance la requête ajax qui a pour fonction response pour afficher la requête La fonction display est ainsi créée:
            let jsonDatas = JSON.parse(response);
            console.log(jsonDatas); //La réponse de la requête doit arriver en JSON

            let firstarg=this.button[i];
